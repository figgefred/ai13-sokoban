...
public class Tunnel {
    ...
    public Path getPath(BoardPosition from, BoardPosition to)
    {
        Iterator<BoardPosition> iter;
        boolean reverse = false;
        if(Positions.peekFirst().equals(from))
        {
            if(to == null && Path != null && Path.getPath().size() == Positions.size())
                return Path;
            iter = Positions.iterator();
        }
        else if(Positions.peekLast().equals(from))
        {
            if(to == null && RevPath != null && RevPath.getPath().size() == Positions.size())
                return RevPath;
            iter = Positions.descendingIterator();
            reverse = true;
        }
        ...
        {
            BoardPosition tmp = iter.next();
            if(to != null && tmp.equals(to))
            {
                stop = true;
            }
            path.append(tmp);
    ...
    public String toString()
    {
        ...
        {
            for(int c = 0; c < StartingState.getColumnsCount();c++)
            {
