...
public class SingleBlockPlayer {
	...
	public SingleBlockPlayer(LiveAnalyser liveAn, Analyser analyser) {			
		this.analyser = analyser;
		this.pathfinder = new PathFinder();
                this.liveAnalyser = liveAn;
	}
	...
	public List<Move> findGoalMoves(Move root, BoardPosition block) {
		...
		for(BoardPosition goal : board.getGoalNodes()) {		
			...
			SingleBlockMove initial = new SingleBlockMove(liveAnalyser, analyser, pathfinder, block, goalindex);
			initial.board = clone;		
			...
			if(move != null) {
				Move normalised = new Move(root.liveAnalyser, root.analyser, root.pathfinder);
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		PathFinder pathfinder = new PathFinder();
                LiveAnalyser l = new LiveAnalyser( pathfinder);
                Analyser analyser = new Analyser(l, board);
		
                
		SingleBlockPlayer noob = new SingleBlockPlayer(l, analyser);
		SingleBlockPlayer.VERBOSE = false;
		
//		BoardPosition block = board.getBlockNodes().get(0);
		for(BoardPosition block : board.getBlockNodes())
		{	
			Move initial = new Move(l, analyser, pathfinder);	
			initial.board = board;
			initial.path = new Path();
