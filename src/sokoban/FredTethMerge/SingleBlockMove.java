...
public class SingleBlockMove extends Move {
	...
	public SingleBlockMove(LiveAnalyser liveAn, Analyser analyser, PathFinder pathfinder, BoardPosition block, int goalindex) {
		super(liveAn, analyser, pathfinder);
		this.block = block;
	...
	public List<Move> getNextPushMoves() {
		...
		for(BoardPosition candidate : pushPositions)
		{
			...
			SingleBlockMove move = new SingleBlockMove(liveAnalyser, analyser, pathfinder, newBoard.getLastPushedBlock(), goalindex);
			move.board = newBoard;
